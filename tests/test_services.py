...
def test_process_bank_search_basic_case(mock_transactions):
    ...
    with patch("src.services.data_frame_filtered_by_date", return_value=mock_transactions):
        ...
        assert len(data) == 2
        assert all(("колхоз" in item["Описание"].lower() or "колхоз" in item["Категория"].lower()) for item in data)
...
def test_process_bank_search_json_structure(mock_transactions):
    ...
    with patch("src.services.data_frame_filtered_by_date", return_value=mock_transactions):
        ...
        assert all(key in item for item in data for key in ["Дата операции", "Описание", "Категория", "Сумма"])
...
@pytest.mark.parametrize(
    ...
    [("колхоз", 2), ("кафе", 1), ("перевод", 1), ("", 3)],  # Пустой запрос должен вернуть все
)
