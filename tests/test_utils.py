...
from src.utils import data_frame_filtered_by_date, get_dataframe, greeting, top_transactions


# Тест-кейсы для утреннего времени (6–12 часов)
@pytest.mark.parametrize("hour,expected_greeting", [(7, "Доброе утро!"), (11, "Доброе утро!")])
...
@pytest.mark.parametrize("hour,expected_greeting", [(13, "Добрый день!"), (17, "Добрый день!")])
def test_day_greeting(hour, expected_greeting):
    date = datetime(year=2023, month=10, day=1, hour=hour)
    result = greeting(date)
    assert result == {
        "greeting": expected_greeting
    }, f"Expected greeting '{expected_greeting}' at {hour}:00 but got {result}"


# Тест-кейсы для вечернего времени (18–24 часа)
@pytest.mark.parametrize("hour,expected_greeting", [(19, "Добрый вечер!"), (23, "Добрый вечер!")])
...
@pytest.mark.parametrize("hour,expected_greeting", [(0, "Доброй ночи!"), (5, "Доброй ночи!")])
...
# Проверка выбора топ-5 транзакций
...
# Проверка случая с менее чем 5 транзакциями
@patch("src.utils.data_frame_filtered_by_date")
...
# Проверка формата возвращаемого JSON
@patch("src.utils.data_frame_filtered_by_date")
def test_top_transactions_json_format(mock_data_frame_filtered_by_date, sample_dataframe):
    ...
    if result["top_transactions"]:
        ...
        assert all(
            "date" in t and "amount" in t and "category" in t and "description" in t
            for t in result["top_transactions"]
        )


# Проверка игнорирования user_date
@patch("src.utils.data_frame_filtered_by_date")
...
# Проверка вызова data_frame_filtered_by_date
@patch("src.utils.data_frame_filtered_by_date")
