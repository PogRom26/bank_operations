...
def process_bank_search(user_date: str = "04.09.2021", search_word: str = "") -> list[dict]:
    ...
    def contains_word(row, word):
        ...
        return any(str(cell).lower().find(word.lower()) != -1 for cell in row if pd.notna(cell))

    filtered_data = data_frame_filtered_by_date()

    mask = filtered_data["Описание"].astype(str).str.contains(search_word, case=False, na=False) | filtered_data[
        "Категория"
    ].astype(str).str.contains(search_word, case=False, na=False)
